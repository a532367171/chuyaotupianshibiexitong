...
namespace 出窑服务图片识别版.common
{
    public class StringHelper
    {
        ...
        #region AES加密算法1
        public static string Encrypt1(string toEncrypt, string key, string iv)
        {
        ...
        public static string Decrypt1(string toDecrypt, string key, string iv)
        {
            byte[] keyArray = UTF8Encoding.UTF8.GetBytes(key);
            byte[] ivArray = UTF8Encoding.UTF8.GetBytes(iv);
        ...
        #region DataTable 转 一维数组
        public static string[] dtToArr1(DataTable dt)
        {
            if (dt == null || dt.Rows.Count == 0) return new string[5];
        ...
        public static string Change(string strChar)
        {
